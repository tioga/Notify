...
public class JarClassLoader extends ClassLoader {
  ...
  public JarClassLoader(ClassLoader parent) {
    ...
    if ("file".equals(protocol)) {
      ...
      if (fileJar.isDirectory()) {
        ...
        for (String fileName : fileJar.list()) {
          logInfo(LogArea.JAR, "  %s", fileName);
        }
  ...
  private File createTempFile(JarEntryInfo inf)
    ...
    try {
      ...
      logInfo(LogArea.JAR, "Created temp file: %s", fileTmp.getAbsolutePath());
   ...
   * @return Loaded class.
   * @throws JarClassLoaderException on failure
   */
      ...
      throws ClassNotFoundException
  {
    ...
    //
    // Setting thread context class loader for the top thread in invokeStart()
    // method is sufficient for most cases. It fails for new threads created
    // not from the start thread.
    //
...
//      System.err.printf("JarClassLoader-" + level + ": " + sMsg + "\n", obj);
//    } else {
