...
import org.lqnotify.kernel.AbstractStore;
import org.lqnotify.kernel.config.CouchServers;
import org.lqnotify.kernel.config.CouchServersConfig;
...
public class DomainStore extends AbstractStore {
  ...
  private void deleteDomain(String domainName) {
    ...
    if (response.isNotEmpty()) {
      ...
      couchDatabase.delete()
          ...
          .onError(r -> log.error(String.format("Failure deleting domain %s from master db [%s] - %s", domainProfile.getDomainName(), r.getHttpStatus(), r.getErrorReason())))
          .execute();
    }
  ...
  private void createNotifyDatabase(DomainProfileEntity domainProfile) {
    ...
    if (createResponse.isError()) {
      String msg = String.format("Exception creating notification database %s for domain %s: %s", couchDatabase.getDatabaseName(), domainProfile.getDomainName(), createResponse.getHttpStatus());
