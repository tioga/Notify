package org.tiogasolutions.notifyserver.engine.core.v1.admin;
...
import org.tiogasolutions.notifyserver.engine.core.AbstractEngineJaxRsTest;
import org.tiogasolutions.notifyserver.kernel.notification.CreateNotification;
import org.tiogasolutions.notifyserver.kernel.notification.TaskQuery;
import org.tiogasolutions.notifyserver.kernel.task.TaskEntity;
import org.tiogasolutions.notifyserver.pub.Notification;
import org.tiogasolutions.notifyserver.pub.NotificationQuery;
...
import static org.tiogasolutions.notifyserver.kernel.TestFactory.API_KEY;
import static org.tiogasolutions.notifyserver.kernel.TestFactory.DOMAIN_NAME;
...
public class AdminResourceV1Test extends AbstractEngineJaxRsTest {
  ...
  public void beforeClass() throws Exception {
    getExecutionManager().newApiContext(API_KEY);
  ...
  public void test_api_v1_admin_domains_KernelTest() {
    String path = String.format("/api/v1/admin/domains/%s", DOMAIN_NAME);
    Response response = request(target(path)).get();
  ...
  public void test_api_v1_admin_domains_KernelTest_notifications() throws Exception {
    ...
    String path = String.format("/api/v1/admin/domains/%s/notifications", DOMAIN_NAME);
    Response response = request(target(path)).get();
    assertEquals(response.getStatus(), 200);
    ...
    path = String.format("/api/v1/admin/domains/%s/notifications", DOMAIN_NAME);
  ...
  public void test_api_v1_admin_domains_KernelTest_notifications_BadId() {
    String path = String.format("/api/v1/admin/domains/%s/notifications/%s", DOMAIN_NAME, "1");
    Response response = request(target(path)).get();
  ...
  public void test_api_v1_admin_domains_KernelTest_notifications_GoodId() {
    ...
    String path = String.format("/api/v1/admin/domains/%s/notifications/%s", DOMAIN_NAME, ref.getNotificationId());
    Response response = request(target(path)).get();
  ...
  public void test_api_v1_admin_domains_KernelTest_routecatalog() {
    String path = String.format("/api/v1/admin/domains/%s/route-catalog", DOMAIN_NAME);
    Response response = request(target(path)).get();
